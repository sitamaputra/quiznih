...
export default function CreateQuizPage() {
  ...
  useEffect(() => {
    ...
    fetch("https://celo-sepolia.drpc.org", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        jsonrpc: "2.0", method: "eth_getBalance",
        params: [publicKey, "latest"], id: 1,
      }),
    })
      .then((r) => r.json())
      .then((data) => {
...
Berikan HANYA text JSON valid dengan format persis seperti di bawah ini (tanpa markdown formatting, tanpa teks awalan/akhiran):
{
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      console.log("[deposit] rewardType:", rewardType, "celoAmount:", celoAmount);
      if (rewardType === "CELO" && celoAmount > 0) {
        ...
        if (!depositResult.success) {
          ...
          // Simpan contract_quiz_id dan deposit_status ke Supabase
          const { uuidToBytes32 } = await import("@/lib/celo");
          await supabase.from("quizzes").update({
            contract_quiz_id: uuidToBytes32(quizData.id),
            deposit_status: "confirmed",
            deposit_tx: depositResult.txHash,
            escrow_balance: Number(rewardPool),
          }).eq("id", quizData.id);
        }
      }
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-black text-[#FCFF52]">
                          {balance !== null ? `◎ ${balance} CELO` : "Loading..."}
                        </p>
                      </div>
                    </div>
                    {isDevnet && (
